...
{{-- <script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Recalculate amount when linked accounts change
        linkedAccountsInput.addEventListener("input", () => {
            const accounts = linkedAccountsInput.value.split(",")
                .map(a => a.trim())
                .filter(a => a.length > 0);

            // Base amount × (1 + number of linked accounts)
            const total = BASE_AMOUNT * (1 + accounts.length);
            amountInput.value = total.toFixed(2);
        });
        ...
        form.addEventListener("submit", async (e) => {
            ...
            let bodyData = {};
            formData.forEach((value, key) => { bodyData[key] = value; });
            bodyData.amount = Math.round(amount * 100);
            ...
            try {
                ...
                // 💡 Case 1: Backend returned clientSecret (normal Stripe payment)
                if (data.clientSecret) {
                    const { error, paymentIntent } = await stripe.confirmCardPayment(data.clientSecret, {
                        payment_method: {
                            card: cardElement,
                ...
                // 💡 Case 2: Backend already split and marked invoices as paid
                } else if (data.status === "paid" || data.message) {
                    alert(data.message || "Invoices created and marked as paid successfully.");
                    window.location.href = "{{ route('payment.success') }}";
                } else {
                    throw new Error("Unexpected response from server.");
